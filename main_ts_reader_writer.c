...
int main(int argc, char *argv[]) {
  ...
  ts_sem_init(&reader_sem, 1);
  ts_sem_init(&db, 1);

  lock_init_ts(&mutex_reader);
  lock_init_ts(&mutex_writer);
  ...
  for(int i = 0; i < NB_WRITERS; i++) {
    ...
    int error = pthread_create(&(writers[i]), NULL, &ts_writer, (void *) all_writers_args[i]);
  ...
  for(int i = 0; i < NB_READERS; i++) {
    ...
    int error = pthread_create(&(readers[i]), NULL, &ts_reader, (void *) all_readers_args[i]);
    if (error != 0) fprintf(stderr, "pthread_create failed\n");
  }
  ...
  lock_destroy_ts(&mutex_reader);
  lock_destroy_ts(&mutex_writer);

  ts_sem_destroy(&reader_sem);
  ts_sem_destroy(&db);
